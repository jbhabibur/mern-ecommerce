...
export const CategoryProductCard = ({ view }) => {
  const isList = view === "list";

  // Mobile-e grid2 view-te height/width boro korar logic
  return (
    <div
      className={`group flex transition-all duration-300 ${
        isList
          ...
          : "flex-col w-full" // Mobile-e full width hobe
      }`}
    >
      {/* Image Container */}
      <div
        className={`relative overflow-hidden bg-gray-50 shrink-0 transition-all duration-500 ${
          isList
            ...
            : "aspect-[3/4] w-full" // Mobile-e 3/4 ratio dile card lamba ebong boro dekhabe
        }`}
      >
        <img
          src="https://images.unsplash.com/photo-1598454541340-f9a1199b4cc1?q=80&w=400&auto=format&fit=crop"
          alt="Product"
      ...
      <div
        className={`${
          isList ? "flex-1 pt-2" : "mt-3 text-left" // Mobile-e text left align thakle boro mone hoy
        }`}
      >
        <h3 className="font-bold text-gray-800 uppercase tracking-tight line-clamp-2 text-xs md:text-sm">
          Men's Premium Oxford Shirt
        </h3>
        ...
        {isList && (
          <div className="mt-2 md:mt-3">
            <p className="hidden sm:line-clamp-2 md:line-clamp-3 text-xs md:text-sm text-gray-500 leading-relaxed">
              Crafted from premium cotton, this shirt offers both comfort and
              durability.
            </p>
