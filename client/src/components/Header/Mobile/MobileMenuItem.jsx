...
export const MobileMenuItem = ({ label, icon: Icon, hasArrow = false, isBold = true, isCapital = false }) => {
  return (
    <div className="flex items-center justify-between px-4 py-2 border-b border-[1px] border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors">
      <div className="flex items-center gap-3">
        ...
        <span className={`text-sm ${isBold ? 'font-bold' : 'font-normal'} ${isCapital ? "uppercase" : "capitalize"}  tracking-tight text-gray-900`}>
          {label}
        </span>
      </div>
